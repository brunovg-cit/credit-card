    ...
    var body: some View {
        HStack(spacing: 50) {
            AsyncImage(url: URL(string: card.image ?? "")) { phase in
                            ...
                            @unknown default:
                                // This is just to remove warning for Unkown future cases in the switch
                                EmptyView()
                            }
                        }
            VStack(alignment: .center, spacing: 10) {
                Text("\(card.alias ?? "N/A")").fontWeight(.bold)
